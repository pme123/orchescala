...
class C7WorkerRegistry(client: C7WorkerClient, engineType: EngineType = EngineType.C7)
  ...
  override def requiredLayers: Seq[ZLayer[Any, Nothing, Any]] =
    Seq(SharedC7ExternalClientManager.layer(engineType))
    ...
    client.client.asInstanceOf[ZIO[R, Any, ExternalTaskClient]].flatMap { client =>
      acquireReleaseWith(ZIO.succeed(client))(_.closeClient()): client =>
        for
          _                             <-
            logInfo(
              s"Starting $engineType Worker Client - Available Processors: ${Runtime.getRuntime.availableProcessors()}"
            )
          ...
          _                             <- logInfo(s"$engineType Worker Client started - registered ${c7Workers.size} workers")
  ...
  private def registerWorker(worker: C7Worker[?, ?], client: ExternalTaskClient) =
    logDebug(s"""Registering $engineType Worker subscription for topic: '${worker.topic}'
    ...
    private def closeClient(): UIO[Unit] =
      logDebug("Closing C7 Worker Client and all subscriptions") *>
        ZIO.attempt(if client != null then client.stop() else ())
          .tap(_ => logInfo(s"$engineType Worker Client closed successfully"))
          .tapError(err => logError(s"Error closing $engineType Worker Client: $err"))
          .ignore
  end extension
