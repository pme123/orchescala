...
/** Utility for signing and verifying IdentityCorrelation objects.
  *
  * Uses HMAC-SHA256 to create a cryptographic signature that binds the identity to a specific
  * process instance, preventing tampering and replay attacks.
  */
object IdentityCorrelationSigner:
  ...
  /** Sign an IdentityCorrelation with a process instance ID.
    *
    * @param correlation
    *   The correlation to sign (without signature)
    * @param processInstanceId
    *   The process instance ID to bind to
    * @param secretKey
    *   The secret key for HMAC signing
    * @return
    *   A new IdentityCorrelation with signature and processInstanceId set
    */
  def sign(
