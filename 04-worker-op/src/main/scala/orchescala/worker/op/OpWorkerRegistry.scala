...
class OperatonWorkerRegistry(client: OperatonWorkerClient)
    extends WorkerRegistry:

  // Provide the SharedOperatonExternalClientManager layer required by Operaton workers
  override def requiredLayers: Seq[ZLayer[Any, Nothing, Any]] =
    Seq(SharedOperatonExternalClientManager.layer)

  protected def registerWorkers[R](workers: Set[WorkerDsl[?, ?]])(using config: WorkerConfig): ZIO[R, Any, Any] =
    // Cast to Any to match the generic signature, but this will only work if R includes SharedOperatonExternalClientManager
    client.client.asInstanceOf[ZIO[R, Any, ExternalTaskClient]].flatMap { client =>
      acquireReleaseWith(ZIO.succeed(client))(_.closeClient()): client =>
        for
          _                                <-
            logInfo(
              s"Starting Operaton Worker Client - Available Processors: ${Runtime.getRuntime.availableProcessors()}"
            )
          operatonWorkers: Set[OperatonWorker[?, ?]] = workers.collect { case w: OperatonWorker[?, ?] => w }
          _                                <- foreachParDiscard(operatonWorkers)(w => registerWorker(w, client))
                                              .withParallelism(config.engineConfig.parallelism)
          _                                <- logInfo(s"Operaton Worker Client started - registered ${operatonWorkers.size} workers")
          _                                <- ZIO.never // keep the worker running
        yield ()
    }.mapError(_.asInstanceOf[Any])

  private def registerWorker(worker: OperatonWorker[?, ?], client: ExternalTaskClient) =
    logDebug(s"""Registering Operaton Worker subscription for topic: '${worker.topic}'
  ...
  extension (client: ExternalTaskClient)
    private def closeClient(): UIO[Unit] =
      logDebug("Closing Operaton Worker Client and all subscriptions") *>
        ZIO.attempt(if client != null then client.stop() else ())
          .tap(_ => logInfo("Operaton Worker Client closed successfully"))
          .tapError(err => logError(s"Error closing Operaton Worker Client: $err"))
          .ignore
  end extension

  lazy val engineConnectionManagerFinalizer: ZIO[Scope, Nothing, Any] = ZIO.unit
end OperatonWorkerRegistry
